...
export default function OwnerDevicesPage() {
  ...
  const [productType, setProductType] = useState('band');
  ...
  const mint = async () => {
    ...
    setMessage(`${data.productType} ${data.code} is ready`);
    load();
  };
    ...
    <div className="min-h-screen bg-zinc-950 text-white px-6 py-12">
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-white/60 mb-8">
          Make a code, write only this URL on the chip, then the customer claims it.
        </p>

        <div className="bg-zinc-900 rounded-3xl p-6 mb-8 flex flex-wrap gap-4 items-end">
          <label className="block">
            <span className="text-sm text-white/50">Product</span>
            <select
              value={productType}
              onChange={(e) => setProductType(e.target.value)}
              className="block mt-2 bg-zinc-800 rounded-xl px-4 py-3"
            >
